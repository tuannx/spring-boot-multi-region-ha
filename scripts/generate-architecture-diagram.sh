...
# =========================================================
# 1. Aurora / PostgreSQL Multi-Region HA Architecture
# =========================================================
echo "==> [Case 1] Validating Aurora architecture (showcase)..."
...
# =========================================================
# 2. Cassandra Multi-Region Active-Active Architecture
# =========================================================
echo "==> [Case 2] Validating Cassandra architecture (showcase)..."
...
# =========================================================
# 3. Dual Message Flows: HTTP Sync vs Queue Async Sequence
# =========================================================
echo "==> [Flows] Validating Message Flows sequence (showcase)..."
node "${ARCHIFY_BIN}" validate sequence docs/flows.json --quality showcase --json

echo "==> [Flows] Delivering HTML to docs/flows.html..."
node "${ARCHIFY_BIN}" deliver sequence docs/flows.json docs/flows.html --quality showcase --json

echo "==> [Flows] Running visual-check and capturing screenshots..."
node "${ARCHIFY_BIN}" visual-check docs/flows.html --json

if [[ -f "docs/flows.visual-check.1440x900.dark.png" ]]; then
  cp "docs/flows.visual-check.1440x900.dark.png" "docs/assets/flows-dark.png"
  echo "==> Generated docs/assets/flows-dark.png"
fi
if [[ -f "docs/flows.visual-check.1440x900.light.png" ]]; then
  cp "docs/flows.visual-check.1440x900.light.png" "docs/assets/flows-light.png"
  echo "==> Generated docs/assets/flows-light.png"
fi
...
inject_switcher() {
  ...
  elif [[ "$active_tab" == "cassandra" ]]; then
    cas_bg="#2563eb"
  ...
  echo "==> Injected multi-diagram switcher into $file"
}
